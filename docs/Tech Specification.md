...
This document describes the implementation choices for the stock console.

### Framework

React with Vite and TypeScript.

React Router provides client-side routes and browser history.
...
The interface uses project CSS in `src/App.css` and `src/index.css`. Controls use native HTML elements where they provide the required keyboard and accessibility behaviour without adding a component library.
...
- `App` for the application shell and route definitions.
- `StockListPage` for the paginated catalogue.
- `ItemDetailPage` for `/items/:id` and stock correction.

Loading, empty and error states are rendered by the page that owns each request.
...
- On a successful stock correction, update the individual product cache and invalidate product-list queries so they are refetched.
...
The layout uses a restrained, high-contrast palette with dark text, cool neutral surfaces, teal primary actions, amber low-stock indicators and red errors. Spacing and controls are sized for ward tablets, with a minimum target size of 44 by 44 pixels.
...
- Use status and alert roles for loading, save and error messages.
...
Every data-backed screen has loading, empty or not-found, and error states. Error states include a retry action. The API supports slow-request testing with `?delay=2000` and HTTP failures with `/http/500`.
...
DummyJSON simulates product updates but does not persist them on the server. After a successful `PUT /products/{id}`, the application updates the detail cache and invalidates product-list queries so the correction is visible while the current session is active. A full reload may show the original server value again. This is treated as a property of the mock API, not as confirmed persistence.
...
3. **Client-Side Cache Handling for Simulated Product Updates**
   - **Rejected Alternative:** Relying solely on server re-fetches after a `PUT /products/{id}` request.
   - **Reasoning:** DummyJSON does not persist mutations on its server. To provide a correct user experience during testing, we update the individual product cache and invalidate product-list queries after a successful `200 OK` response.
